...
public class UserAdminAction extends ActionSupport implements ModelDriven<User>{
	...
	public String findAll(){
		...
		/*-----------------------后台模糊查询，并由Struts指定显示界面---------------------------*/
	//后台模糊查询用户：
	public String findNameByAddr(){
	/*	PageBean<User> pageBean = userService.findByPage(page);
		
		ActionContext.getContext().getValueStack().set("pageBean", pageBean);*/
		
		
		String add=user.getAddr();
		 List<User> listUser=userService.findNameByAddr(add);
		 
		 PageBean<User> pageBean =new PageBean<User>();
		 pageBean.setList(listUser);
		 ActionContext.getContext().getValueStack().set("pageBean", pageBean);
		 
		 /*ActionContext.getContext().put("pageBean", listUser);*/

		return "findNameByAddr";
